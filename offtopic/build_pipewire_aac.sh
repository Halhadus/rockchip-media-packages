...
prepare_env() {
    ...
    if [ -z "$CI" ]; then
        ...
        run_silent "Updating apt sources (adding non-free components)" sed -i 's/Components: main/Components: main contrib non-free non-free-firmware/g' /etc/apt/sources.list.d/debian.sources
